...
class GridMDP:
    ...
    def start_distribution(self) -> np.ndarray:
        """
        Return the start distribution
        :return:
        """
        dist = np.zeros(shape=(self.x_dim, self.y_dim), dtype=np.float)
        dist[self.start_state[0], self.start_state[1]] = 1.
        return dist
...
class MazeWorld(GridMDP):
    ...
    def __init__(self):
        ...
        self._state = self.start_state
        # self.target_state = [np.asarray([0, 4]), np.asarray([2, 4]), np.asarray([1, 3]), np.asarray([1, 5])]  # , np.asarray([8, 1]), np.asarray([6, 6]), np.asarray([4, 9])]
        self.target_state = [np.asarray([1, 4])]
        self.time_limit = 100
        self._rewards = np.zeros(shape=(self.x_dim, self.y_dim), dtype=np.float)
