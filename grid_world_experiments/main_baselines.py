...
class GAIL:
    ...
    def __init__(self):
        ...
        self.policy = SoftQLearning(x_dim=self.env.dims, out_dim=len(self.env.action_space),
                                    ...
                                    ent_coef=0.3, target_update=3e-2)
    ...
    def plot_dist(self, num_samples=100, it=0, dname='aim'):
        ...
        states, _, _, next_states, _ = self.agent_buffer.sample(num_samples)
        target_dist = np.reshape(self.env.target_distribution(), (-1,))
        start_dist = np.reshape(self.env.start_distribution(), (-1,))
        target_distribution = np.random.choice(target_dist.shape[0], num_samples, p=target_dist)
        start_distribution = np.random.choice(start_dist.shape[0], num_samples, p=start_dist)
        target_distribution = target_distribution.reshape([-1, 1]).astype(np.float32)
        start_distribution = start_distribution.reshape([-1, 1]).astype(np.float32)
        ...
        if self.env.dims > 1:
            ...
            start_distribution = np.concatenate([start_distribution, start_distribution], axis=-1)
            start_distribution[:, 0] = start_distribution[:, 0] // self.env.y_dim
            start_distribution[:, 1] = start_distribution[:, 1] % self.env.y_dim
        ...
        if self.env.dims == 1:
            ...
            start_vals, start_counts = np.unique(start_distribution, axis=0, return_counts=True)
            ...
            start_counts = start_counts.astype(np.float) / np.max(start_counts)

            dist = self.generate_state_distribution()
            ...
            plt.scatter(target_vals[:, 0], target_vals[:, 1], 200 * target_counts,
                        color='r', alpha=1.0, label='target')
            plt.scatter(start_vals[:, 0], start_vals[:, 1], 200 * start_counts, color='g', alpha=1.0, label='start')

            plt.plot([-0.5, 3.5, 3.5], [2.5, 2.5, 7.5], marker='', color='k', linewidth=3)

            plt.imshow(dist.T, cmap=plt.cm.jet, norm=colors.Normalize(vmin=0.0, vmax=0.09), origin='lower')
        ...
        # plt.title(f'Density for agent, start and target distributions state Iteration {it}')
        # plt.show()
        plt.tight_layout()
        plt.axis('off')
        plt.savefig(f'{dname}/s_{it}.png', dpi=300, bbox_inches='tight')
        # exit()
        plt.cla()
        plt.clf()
        plt.close()
        ...
        plt.scatter(target_vals[:, 0] + 0.5, target_vals[:, 1] + 0.5, 200 * target_counts,
                    color='r', alpha=1.0, label='target')
        plt.scatter(start_vals[:, 0] + 0.5, start_vals[:, 1] + 0.5, 200 * start_counts, color='g', alpha=1.0, label='start')

        plt.plot([0, 4, 4], [3, 3, 8], marker='', color='k', linewidth=3)
