...
class Discriminator(nn.Module):
    def __init__(self, x_dim=1, reward_type='aim', scale=10.0):
    ...
    def forward(self, x: Union[np.ndarray, torch.Tensor], inf=False) -> torch.Tensor:
        """
        ...
        """
        x = self.scale * x
        # print('Input to disc:', x)
        if type(x) is np.ndarray:
            x = torch.from_numpy(x)
        if not inf and self.use_cuda:
            x = x.cuda()
        output = self.d(x)
        return output
    ...
    def reward(self, x: np.ndarray) -> np.ndarray:
        ...
        r = self.forward(x, True)
        if self.reward_type is not None:
            r = self.reward_type(r)
        # r = ((r - self.max_r)/(self.max_r - self.min_r))
    ...
    def optimize_discriminator(self, target_states: np.ndarray, policy_states:  np.ndarray,
        ...
        """
        self.d.to(self.device)
        # print('target states: ', target_states[:10])
        # print("--------------------")
        # print(target_states.shape)
        # print(policy_states.shape)
        # num_samples = 50
        ...
        if self.reward_type != wasserstein_reward:
            ...
            # ####### WGAN loss
            # print('ones: ', ones[0:10])
            pred_ones = self(ones)
            # print('pred_ones: ', pred_ones[0:10])
            pred_zeros = self(zeros)
            # print('pred ones', pred_ones)
            # print('pred zeros', pred_zeros)
