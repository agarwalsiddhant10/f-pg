...
class GoalReachingEnv(object):
    ...
    def _get_obs(self):
        ...
        achieved_goal = np.array([achieved_goal[0] / (self._maze_size_scaling * self._np_maze_map.shape[1]),
                                    achieved_goal[1] / (self._maze_size_scaling * self._np_maze_map.shape[0])])
        # print('modified achieved goal: ', achieved_goal)

        desired_goal = self._goal
        # print('Desired goal: ', desired_goal)
        desired_goal = np.array([desired_goal[0] / (self._maze_size_scaling * self._np_maze_map.shape[1]),
                                    desired_goal[1] / (self._maze_size_scaling * self._np_maze_map.shape[0])])
        # print('modified desired goal: ', desired_goal)
        if self.eval:
            obs = {
    ...
    def step(self, a):
        ...
        obs = self._get_obs()
        return obs, reward, done, {'is_success': done, 'reward': np.linalg.norm(self.get_xy() - self.target_goal)}
