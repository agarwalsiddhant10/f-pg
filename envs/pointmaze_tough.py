...
class MazeEnvTough(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,
                 maze_spec=U_MAZE_TOUGH,
                 reward_type='sparse',
                 reset_target=False,
                 seed=0):
        ...
        self.reset_locations = list(zip(*np.where(self.maze_arr == INITIAL)))
        ...
        self.range_lim_x = (0, self.maze_arr.shape[1]-1)
        self.range_lim_y = (0, self.maze_arr.shape[0]-1)

        # print("MazeEnv: range_lim_x: ", self.range_lim_x)
        # print("MazeEnv: range_lim_y: ", self.range_lim_y)
        ...
        #     raise ValueError("More than 1 goal specified!")
            ...
            print('No goal locations present')
    ...
    def normalize_obs(self, obs):
        # print("normalize")
        # print(self.range_lim_x)
        obs = np.copy(obs)
        obs[0] /= self.range_lim_x[1]
        obs[1] /= self.range_lim_y[1]
        obs[2] = (obs[2] + 5.0)/10.0
        obs[3] = (obs[3] + 5.0)/10.0

        target = self._target.copy()
        target[0] /= self.range_lim_x[1]
        target[1] /= self.range_lim_y[1]
        return obs, target
    ...
    def step(self, action):
        ...
        # if reward > 0.0:
        #     print('In MazeEnv, reward is', reward, 'ob is', ob, 'target is', self._target)
        done = False
        norm_obs, norm_target = self.normalize_obs(ob)
        obs = {
            'observation': norm_obs,
            'achieved_goal': norm_obs[0:2],
            'desired_goal': norm_target,
        }
        # print(reward)
        return obs, reward, done, {'is_success': reward > 0.0, 'reward': np.exp(-np.linalg.norm(ob[0:2] - self._target))}
    ...
    def set_target(self, target_location=None):
        ...
        if target_location is None:
            ...
            # print(f'Target location: {reset_location}')
            target_location = reset_location + self.np_random.uniform(low=-.1, high=.1, size=self.model.nq)
        self._target = target_location
        self.goal = np.copy(self._target)
        self.goal[0] /= self.range_lim_x[1]
        self.goal[1] /= self.range_lim_y[1]
    ...
    def reset_model(self):
        ...
        # print(f'Reset location: {reset_location}')
    ...
    def reset(self, new_goal=True, init=None, goal=None, get_init=False):
        ...
        if init is None:
            ...
            # print('Resetting to location: ', init)
            init = [init[0] * self.range_lim_x[1], init[1] * self.range_lim_y[1]]
            return self.reset_to_location(init)
